...
ranking_sums <- results["Please rank the following marketing ideas in order of your sense of their importance & efficacy"] |>
  na.omit() |>
  tidyr::separate_rows(1, sep = ", ") |>
  dplyr::mutate(Position = rep(1:13, each = 8)) |> # Because there are 13 valid obs with 8 items ranked in each
  dplyr::group_by(Position) |>
  dplyr::group_map(\(...) {
    dplyr::mutate(..1, Position = dplyr::row_number())
  }) |> 
  dplyr::bind_rows() |> 
  dplyr::group_by(`Please rank the following marketing ideas in order of your sense of their importance & efficacy`) |> 
